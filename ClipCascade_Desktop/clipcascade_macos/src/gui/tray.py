...
class TaskbarPanel:
    ...
    def create_menu(self):
        ...
        if self.app_dir is not None:
            menu_items.insert(1, item("Program Files", self._on_program_files))
    ...
    def _on_program_files(self, icon, item):
        try:
            subprocess.run(["open", self.app_dir])
        except Exception as e:
            CustomDialog(
                f"Failed to open the file location. Here is the path: {self.app_dir}\nError: {e}",
                msg_type="error",
            ).mainloop()
