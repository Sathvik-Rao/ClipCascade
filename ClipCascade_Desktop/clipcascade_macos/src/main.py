        ...
        if first_connection_lost_notification:
            if notification_enabled:
                notification_queue.put(
                    (
                        "ClipCascade: ⛓️‍💥 Lost WebSocket Connection",  # title
                        "Check your internet connection. Retrying...",  # message
                    )
                )
...
def notification_handler():
    ...
    try:
        while True:
            title, message = notification_queue.get()  # wait for notification
...
def re_connect(
    ...
    if not first_connection_lost_notification:
        if notification_enabled:
            notification_queue.put(
                (
                    "ClipCascade: 🔗 Restored WebSocket Connection",  # title
...
if __name__ == "__main__":
    try:
        # Get the path to the Application Support directory for the current user
        app_dir = os.path.join(
            os.path.expanduser("~"), "Library", "Application Support", "ClipCascade"
        )
        ...
        try:
            lock_file = open(app_dir + "/clipcascade.lock", "w")
            ...
            if run_anyway == "yes":
                os.remove(app_dir + "/clipcascade.lock")
                lock_file = open(app_dir + "/clipcascade.lock", "w")
                fcntl.flock(lock_file, fcntl.LOCK_EX | fcntl.LOCK_NB)
                is_locked = False
            else:
        ...
        LOG_FILE_NAME = app_dir + "/clipcascade_log.log"
        DATA_fILE_NAME = app_dir + "/DATA"
            ...
            while True:
                ...
                if login_successful:
                    ...
                    if ws_conn_successful:
                        startup_login_execution = False
                        CustomDialog(
                            "Success! ClipCascade will now run in the menu bar.",
                            msg_type="success",
                            timeout=5000,
                        ).mainloop()
                        break
        ...
        try:
            ...
            if response_data["macos"] != APP_VERSION:
                new_version_available = [True, response_data["macos"], APP_VERSION]
        except Exception as e:
            # Silent catch
            pass
        ...
        if notification_enabled:
            threading.Thread(target=notification_handler, daemon=True).start()
        ...
        notification_queue.put((None, None))  # exit notification handler
        if is_locked:
            lock_file.close()
        else:
